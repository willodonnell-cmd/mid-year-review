...
const requiredPaths = ['/', 'https://pldv-portfolio-dashboard-jul27.prologis.chatgpt.site/', '/portfolio', '/business-unit-acceleration', '/customer-impact', '/business-building', '/scaling-innovation', '/appendix']
...
const failures = [
  ...
  ...['PLDV Portfolio Returns'].filter((text) => !manifest.includes(text)).map((text) => `Missing required navigation label: ${text}`),
